...
When submitting to App Store Connect, the "Notes for Review" field is crucial:

```
DEMO ACCOUNT:
Email: test-review@privy.io
Password/OTP: XXXXXX

TESTING INSTRUCTIONS:
1. Open app and tap "Get Started"
2. Enter demo email above
3. Enter verification code XXXXXX
4. App will create a Solana wallet for testing
5. Tap "Receive" to see wallet address
6. Tap "Send" to see transaction flow (no real funds needed for demo)

Note: This app allows users to create self-custody Solana wallets. 
No real cryptocurrency is required for review—the demo account 
...
I'd like to clarify how [App Name] addresses guideline 3.1.5:

1. ORGANIZATION ENROLLMENT: We are enrolled as [Company Name], 
   D-U-N-S number [NUMBER].

2. WALLET FUNCTIONALITY: Our app provides self-custody wallet 
   functionality, allowing users to manage their own Solana tokens. 
   We do not take custody of user funds at any point.

3. TEST ACCOUNT: To test the app without requiring cryptocurrency, 
   please use:
   - Email: test-review@privy.io
   - Verification code: XXXXXX
   
   This will create a fully functional test wallet.

4. NO IN-APP PURCHASES REQUIRED: Users can use the app's core 
   functionality without any purchases. Optional NFT purchases 
   use user's own cryptocurrency, not Apple payment systems.

Please let me know if you need any additional information or 
would like to schedule a call to discuss.
```
...
<ArticleSection name="Regional Strategies" id="regional-strategies" level="h2" />

Not all App Store regions have the same scrutiny.

### High-scrutiny regions
- United States
- European Union
- United Kingdom
- Japan

### Moderate-scrutiny regions
- Canada
- Australia
- Singapore
- Hong Kong
...
1. Submit to a moderate-scrutiny region first
2. Build a track record of compliant operation
3. Expand to high-scrutiny regions with that history

This isn't about dodging rules—it's about building evidence that your app operates responsibly.
